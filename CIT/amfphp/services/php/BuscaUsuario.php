...
Class BuscaUsuario{
function teste($s){
	$r=preg_replace( '/[`^~\'"]/', null, iconv( 'UTF-8', 'ASCII//TRANSLIT', $s ) );
	return $r;
}

function semAcentuacao($s){
...
//SELECT `Nome`,`perfil`,`lotacao`,`ramal`,`email`,`spark` FROM `usuario` WHERE `Nome` LIKE '%lídia%'
	...
	$sql = "SELECT id_usuario, nome , lotacao, ramal, email, spark FROM usuario WHERE Nome LIKE '%".$string."%' ";
